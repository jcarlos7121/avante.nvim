...
function M.launch_rag_service(cb)
  ...
  if M.get_rag_service_runner() == "docker" then
    ...
    local cmd_ = string.format(
      "docker run --platform=linux/amd64 -d -p %d:20250 --name %s -v %s:/data -v %s:/host:ro --add-host=host.docker.internal:host-gateway -e ALLOW_RESET=TRUE -e DATA_DIR=/data -e RAG_PROVIDER=%s -e %s_API_KEY=%s -e %s_API_BASE=%s -e RAG_LLM_MODEL=%s -e RAG_EMBED_MODEL=%s %s %s",
      port,
      container_name,
      data_path,
